...
ENDINTERFACE.
""Sortierte Liste hab ich eingefügt
"! Abstrakte Klasse für die Klassen <strong>lcl_passenger_flight</strong> und <strong>lcl_cargo_flight</strong><br/>
"! {@link .lcl_passenger_flight} und <br/>
"! {@link .lcl_cargo_flight}<br/>
"! Jede Instanz wird dutch folgende Attribute eindeutig identifiziert:<br/>
"! <ol>
"! <li>{@link .lcl_flight.data:carrier_id},</li>
"! <li>{@link .lcl_flight.data:connection_id},</li>
"! <li>{@link .lcl_flight.data:flight_date}</li>
"! </ol>
...
"! Flight Carrier
"! Eine factory Logik stellt sicher, dass nur eine Instanz pro CARRID erstellt wird.
...
""" Übung 26
    "! factory Methode gibt eine Instanz der Klasse zurück
    "! @parameter i_carrier_id | Dreistellige Identifikation des CARRID
    "! @parameter r_result | Referenz auf die Instanz
    "! @raising zcx_04_failed | Ausnahmetext bei fehlgeschlagener Instanzierung
    CLASS-METHODS get_instance
      IMPORTING
                i_carrier_id    TYPE /dmo/carrier_id
      RETURNING VALUE(r_result) TYPE REF TO lcl_carrier

      RAISING  zcx_04_failed.
    ...
    "! Suche nach einem <strong>Passagierflug</strong> zwischen 2 Flughäfen
    "! <ul>
    "! <li>Der Flug liegt am oder nach dem Datum</li>
    "! <li>Hat eine Mindestzahl verfügbarer Plätze frei</li>
    "! </ul>
    "! <ol>
    "! <li>Sortierte Liste Item 1</li>
    "! <li>Item 2</li>
    "! </ol>
    "! @parameter i_airport_from_id |<em>Abflughafen</em>
    "! @parameter i_airport_to_id |<em>Zielflughafen</em>
    "! @parameter i_from_date |Erstes mögliches Flugdatum
    "! @parameter i_seats |Mindestzahl der verfügbaren Plätze
    "! @parameter e_flight |Gefundener Flug
    "! @parameter e_days_later |Anzahl der Tage nach dem Wunschdatum
...
**"" Übung 20 neu
  ...
  METHOD get_instance.
   ...
   "" Authority check. Erst Checken, dann lesen
    AUTHORITY-CHECK OBJECT '/LRN/CARR'
            ID '/LRN/CARR' FIELD i_carrier_id
            ID 'ACTVT' FIELD '03'.

    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_04_failed
        EXPORTING
              textid     = zcx_04_failed=>carrier_no_read_auth
*              previous   =
              carrier_id = i_carrier_id.
    ENDIF.

     SELECT SINGLE
      FROM /dmo/carrier  ""/lrn/I_Carrier als Test, hier wird Auth-CHeck gemacht über DataCore Auth.-CHeck über Annotation
    ...
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_04_failed
            EXPORTING
              textid     = zcx_04_failed=>carrier_not_exist
*              previous   =
              carrier_id = i_carrier_id.
