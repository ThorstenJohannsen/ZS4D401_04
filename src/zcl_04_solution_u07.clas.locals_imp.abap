...
CLASS lcl_passenger_flight IMPLEMENTATION.
  ...
  METHOD class_constructor.
    ...
    LOOP AT connection_buffer INTO DATA(connection).
      ...
      """ Zusatzaufgabe wegen Flüge über nach Mitternacht
      IF departure_utclong < arrival_utclong.
        connection-duration = utclong_diff( high = arrival_utclong
                                            low  = departure_utclong ) / 60. "" durch 60 damit wir Minuten bekommen
      ELSE.
        connection-duration = utclong_diff( high = departure_utclong
                                              low  = arrival_utclong ) / 60.
      ENDIF.
  ...
  METHOD get_flights_by_carrier.
    ...
    FIELDS * WHERE carrier_id = @i_carrier_id
...
CLASS lcl_carrier IMPLEMENTATION.
  ...
  METHOD get_output.
    ...
    APPEND |Carrier { me->name } | TO r_result ##NO_TEXT.
    APPEND |Passenger Flights:  { lines( passenger_flights ) } | TO r_result ##NO_TEXT.
    APPEND |Average free seats: { get_average_free_seats(  ) } | TO r_result ##NO_TEXT.
    APPEND |Cargo Flights:      { lines( cargo_flights     ) } | TO r_result ##NO_TEXT.
